...
module.exports = {
  ...
  async execute(interaction, client) {
    async function createTicket(ticketType, reason) {
      console.log(client.config.ticketNameOption)
          ...
          .setDescription(
            ticketType.customDescription ? ticketType.customDescription
              ...
              client.embeds.ticketOpened.description
                ...
                .replace('REASON', reason))
          .setFooter({
            text: "NEX ROLEPLAY" + client.embeds.ticketOpened.footer.text.replace("NEX ROLEPLAY", ""), // Please respect the LICENSE :D
    ...
    if (interaction.isSelectMenu()) {
      if (interaction.customId === "selectTicketType") {
        ...
        if (!ticketType) return console.error(`Tipo de Ticket ${interaction.values[0]} não encontrado!`);
        if (ticketType.askReason) {
          ...
          createTicket(ticketType, "Nenhuma motivo fornecida");
        };
      };
      ...
      if (interaction.customId === "removeUser") {
        ...
        interaction.update({
          content: `> ${interaction.values.length < 1 ? interaction.values : interaction.values.map(a => `<@${a}>`).join(', ')} Removeu o ticket`,
          components: []
        }).catch(e => console.log(e));
      };
    };
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === "askReason") {
        ...
        if (!ticketType) return console.error(`O tipo de Ticket ${interaction.values[0]} não encontrado!`);
